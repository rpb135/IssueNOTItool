...
class ZetVocManager:
    ...
    def ZET_User_BoardSearch(self):
        ...
        with requests.Session() as s:
            ...
            #board_list_url = 'URL주소/qna?&search=&page='+ str(Board_pageNum) + '&size='+ str(Board_sizeNum) +'&recieve=true&complete=true'
    ...
    def JIRA_User_Issue_Create(self):
        ...
        jira_url = 'JIRA REST API 주소/rest/api/2/issue'
        auth = ('계정정보', 'LWnzIgcIMviFBGI5Lg8x327D')
        ## jira API token : LWnzIgcIMviFBGI5Lg8x327D
        headers = {
            'Content-Type': 'application/json'
        }
        ...
        newIssue = json.dumps({
            "fields": {
                ...
                "issuetype": {
                    "id": "10009" #10006 #10009 #https://URL/rest/graphql/1/ 에서 ID값 확인가능
                    #"id": "10009" #10000 스토리 10006 기능 특징 10007 개별업무
                },
    ...
    def JIRA_Issue_IMG_Uproad(self, img_url, issueNum):
        ...
        for idx, (key, elem) in enumerate(img_url.items()):
            if elem is None or elem == "":
                ...
                jira_url = 'URL주소/rest/api/2/issue/' + issueNum + '/attachments'
                auth = ('계정정보', 'LWnzIgcIMviFBGI5Lg8x327D')
                headers = {
    ...
    def JIRA_Issue_Search(self):
        print("이슈 검색 >>")
        jira_url = 'URL주소/rest/api/2/search'
        auth = ('계정정보', 'LWnzIgcIMviFBGI5Lg8x327D')
        ## jira API token : LWnzIgcIMviFBGI5Lg8x327D
        headers = {
            'Content-Type': 'application/json'
    ...
    def JIRA_SendTo_Slack(self, send_issueData):
        ...
        ################################################################################
        account_url = 'URL 주소/wiki/rest/api/user'
        auth = ('계정정보', 'LWnzIgcIMviFBGI5Lg8x327D')
        ## jira API token : LWnzIgcIMviFBGI5Lg8x327D
        headers = {
            'Content-Type': 'application/json'
        ...
        slack_webhook_url = "https://hooks.slack.com/services/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"  #wisestone test
        #slack_webhook_url = "https://hooks.slack.com/services/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" #Zet devops test
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": "* 이슈 번호 : <https://JIRA REST API 주소/browse/"+ send_issueData['jira_issueNum'] + "|[" + send_issueData['jira_issueNum'] +"]>* \n *우선 순위 : " + send_issueData['jira_priority'] + "* \n *담당자 : " + send_issueData['jira_assignee'] + "*"
                    }
                },
                {
                    "type": "section",
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": "*이슈 제목 : <https://JIRA REST API 주소/browse/"+ send_issueData['jira_issueNum'] +"|" + send_issueData['jira_summary'] +">*"
                    },
                },
            ]
        }
    ...
    def ZET_Admin_BoardSearch(self):
        with requests.Session() as s:
            ...
            #Admin_board_list_url = 'https://URL 주소/board?&search=&page='+ str(Board_pageNum) + '&size='+ str(Board_sizeNum) +'&recieve=true&complete=true'
    ...
    def Admin_JIRA_Issue_Create(self):
        jira_url = 'https://JIRA REST API 주소/rest/api/2/issue'
        auth = ('계정정보', 'LWnzIgcIMviFBGI5Lg8x327D')
        ## jira API token : LWnzIgcIMviFBGI5Lg8x327D
        headers = {
            'Content-Type': 'application/json'
    ...
    def Admin_Issue_Search(self):
        print("관리자 이슈 검색")
        jira_url = 'https://JIRA REST API 주소/rest/api/2/search'
        auth = ('계정정보', 'LWnzIgcIMviFBGI5Lg8x327D')
        ## jira API token : LWnzIgcIMviFBGI5Lg8x327D
        headers = {
            'Content-Type': 'application/json'
